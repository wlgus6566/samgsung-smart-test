...
export default function ConferenceDomesticsList({ initialData }) {
  ...
  const { data, error, isLoading } = useSWR(
    `/api/v1/competition/domestic?searchWord=${encodeURIComponent(searchWord)}&currentPage=${currentPage}&size=10`,
    fetcher,
    {
  ...
  return (
    <Contents title="국내" backgroundImage="/images/conference/kv_sports.png">
      <div className="flex justify-between w-full mb-[20px]">
        <h2 className="text-2xl font-bold">국내 대회 요강</h2>
        <Input
          type="search"
          className="w-[426px]"
          value={input}
                ...
                {data.list.map((item, index) => (
                  <tr key={item.competitionSeq}>
                    ...
                    <td className="text-left">
                      <Link
                        href={`${searchParams ? `/conference/domestic/${item.competitionSeq}?${searchParams}` : `/conference/domestic/${item.competitionSeq}`}`}
