...
export default function ConferenceResultsList({ initialData }) {
  ...
  const { data, error, isLoading } = useSWR(
    `/api/v1/bbs/EVENT?searchWord=${encodeURIComponent(searchWord)}&currentPage=${currentPage}&size=10`,
    fetcher,
    {
  ...
  return (
    <Contents title="행사안내" backgroundImage="/images/event/event_kv_bg.jpg">
              ...
              <tbody>
                {data.list.map((item, index) => (
                  <tr key={item.boardSeq || `result-${index}`}>
                    ...
                    <td>
                      <Link
                        href={`${searchParams ? `/event/info/${item.boardSeq}?${searchParams}` : `/event/info/${item.boardSeq}`}`}
